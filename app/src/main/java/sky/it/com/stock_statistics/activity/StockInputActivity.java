 ...
 * @company Name:
 * @author: sky
 * @creatDate: 2019/5/12 9:10 PM
 * @className: StockInputActivity
 * @description:
 * @modified By:
 * @modifyDate:
 */
public class StockInputActivity extends BaseActivity implements View.OnClickListener {
    ...
    private static final String TAG = "StockInputActivity";
     ...
     * 是否已经选择了图片
     */
    private boolean hasSelectedIcon = false;
    ...
    private void initView() {
        ...
        //为保证一个仓库、一个姓名只能对应一组序号，组装key的规则
        number_count = stock_manager + "&" + stock_name + "&number";
        //默认单位，便于输入
        stock_unit = (String) SPUtil.getInstance().get("stock_unit", "片");
        // TODO: 2019/7/31
        etStockUnit.setText(stock_unit);

        stock_number = (int) SPUtil.getInstance().get(number_count, 0);
        //本次的序号
        int thisNum = stock_number + 1;
        txtStockNumber.setText(thisNum + "");
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_save_info:
                ...
                if (TextUtils.isEmpty(stock_manager)) {
                    ...
                    //将本次的序号保存到SharedPreferences中
                    SPUtil.getInstance().put(number_count, stock_number);
                    //保存本次输入的单位
                    SPUtil.getInstance().put("stock_unit", stock_unit);

                    Toast toast = Toast.makeText
                            (this, "数据保存成功！", Toast.LENGTH_LONG);
                    ...
                    //清空相关输入框
                    etStockProductName.setText("");
                    etStockAmount.setText("");
                    etStockType.setText("");
            ...
            case R.id.iv_stock_select:
                ...
                adapter = new IconListAdapter(list, StockInputActivity.this);
